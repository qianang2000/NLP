...
def naiveSoftmaxLossAndGradient(
    ...
    outsideVectors -- outside cvectors is
    ...
    weight = np.array([np.exp(np.dot(x, centerWordVec)) for x in outsideVectors])
    gradCenterVec = outsideVectors.T@weight/sum(weight) - outsideVectors[outsideWordIdx]
    gradOutsideVecs = weight.reshape(outsideVectors.shape[0], 1) * centerWordVec.reshape(1, outsideVectors.shape[1]) / sum(weight)
...
def negSamplingLossAndGradient(
    ...
    real = -np.log(sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec)))
    loss = real - sum([np.log(sigmoid(-np.dot(outsideVectors[w], centerWordVec))) for w in negSampleWordIndices])
    real_center = (sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec)) - 1) * outsideVectors[outsideWordIdx]
    gradCenterVec = real_center + sum([(1 - sigmoid(-np.dot(outsideVectors[w], centerWordVec))) * outsideVectors[w] for w in negSampleWordIndices])
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for i in range(2*windowSize):
        l, gcv, gov = word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]], word2Ind[outsideWords[i]], outsideVectors, dataset)
        loss += l
